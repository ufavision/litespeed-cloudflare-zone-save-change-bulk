...
LOG_CFOFF="/var/log/lscwp-cf-save-cfoff.log"          # Cloudflare ปิดอยู่ใน plugin
...
log_result() {
    ...
    case "$1" in
        ...
        haszone)  ( flock 206; echo "[$ts] $2" >> "$LOG_HASZONE"
> "$LOG_CFOFF" ) 206>"${LOG_FILE}.haszone.lock" ;;
        cfoff)    ( flock 207; echo "[$ts] $2" >> "$LOG_CFOFF"   ) 207>"${LOG_FILE}.cfoff.lock" ;;
    esac
}
...
cleanup() {
    wait
    rm -rf "$RESULT_DIR"
    rm -f "$LOCK_FILE" "${LOG_FILE}.pass.lock" "${LOG_FILE}.fail.lock" "${LOG_FILE}.skip.lock" "${LOG_FILE}.mismatch.lock" "${LOG_FILE}.notcf.lock" "${LOG_FILE}.haszone.lock" "${LOG_FILE}.cfoff.lock"
}
trap cleanup EXIT
...
process_site() {
    ...
    # ── Skip /public_html/ root (ไม่มี domain ต่อท้าย) ──────────
    ...
    case "$STATUS" in
        HAS_ZONE)
            ...
            ;;
        NOPLUGIN)
            _log  "⏭  SKIP (เว็บใหม่ยังไม่ได้ขึ้น หรือ plugin ไม่ active): $LABEL"
            _log_r skip "$SITE | เว็บใหม่ยังไม่ได้ขึ้น หรือ plugin ไม่ active"
            touch "${RESULT_DIR}/skip_${UNIQ}"
            ;;
        CF_OFF)
            _log  "🔴 CF_OFF: $LABEL | Cloudflare ปิดอยู่ใน plugin"
            _log_r cfoff "$SITE | cdn-cloudflare=OFF"
            touch "${RESULT_DIR}/cfoff_${UNIQ}"
            ;;
        NO_CRED)
            local KL NM
...
export LOG_FILE LOCK_FILE LOG_PASS LOG_FAIL LOG_SKIP LOG_MISMATCH LOG_NOTCF LOG_HASZONE LOG_CFOFF RESULT_DIR WP_TIMEOUT MAX_RETRY RETRY_DELAY
